...
def IsGoodLogBuffer(_buffer, _offset, count):
    ...
    length = struct.unpack_from(
        "I", buffer(_buffer, _offset + headerLen - 4 - 4, 4))[0]
...
def DecodeBuffer(_buffer, _offset, _outbuffer):
    ...
    seq = struct.unpack_from(
        "H", buffer(_buffer, _offset + headerLen - 4 - 4 - 2 - 2, 2))[0]
    begin_hour = struct.unpack_from(
        "c", buffer(_buffer, _offset + headerLen - 4 - 4 - 1 - 1, 1))[0]
    end_hour = struct.unpack_from(
        "c", buffer(_buffer, _offset + headerLen - 4 - 4 - 1, 1))[0]
    ...
    try:
        ...
        if MAGIC_COMPRESS_START == _buffer[_offset]:
            ...
            while len(tmpbuffer) > 0:
                single_log_len = struct.unpack_from("H",
                                                    buffer(tmpbuffer, 0, 2))[0]
